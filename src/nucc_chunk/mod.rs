...
pub use nucc_chunk_anm::{Entry, EntryFormat, Curve, CurveHeader, CurveFormat};
pub use nucc_chunk_anm::{ParentChildIndex, ClumpCoordIndex};
...
pub trait NuccChunk: Downcast {
    ...
    {
        Self::read(input, (deku::ctx::Endian::Big, version)).map(|(rest, value)| (rest, Box::new(value) as Box<dyn NuccChunk>))
    }
    ...
    {
        Self::write(&boxed.downcast::<Self>().map(|c| *c).ok().unwrap(), output, (deku::ctx::Endian::Big, version))
    }
}
...
impl NuccChunkType {
    pub fn read_struct<'a>(
        ...
        match NuccChunkType::from_str(chunk_type).unwrap_or_default() {
            ...
            NuccChunkType::NuccChunkUnknown => Ok((input, Box::new(NuccChunkUnknown {data: input.to_bitvec().into_vec(), chunk_type: chunk_type.to_string(), version}))),
        }
    }
    ...
    pub fn write_struct<'a>(
        boxed: Box<dyn NuccChunk>,
        version: u16,
    ) -> Result<DekuBitVec, DekuError> {
        let mut output = DekuBitVec::new();
        match boxed.chunk_type() {
            NuccChunkType::NuccChunkNull | NuccChunkType::NuccChunkIndex => Ok(()) as Result<(), DekuError>,
            ...
            NuccChunkType::NuccChunkUnknown => {
                let mut chunk = boxed.downcast::<NuccChunkUnknown>().map(|c| *c).ok().unwrap();
